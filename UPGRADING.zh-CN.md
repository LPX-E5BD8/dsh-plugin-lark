...
| v0.9.4 | 使用相同持久化状态，但其可构造插件入口可能在 root unload 时丢失异步 disposer。pending 问题可能看似仍可交互，但进程内状态已经消失；对应工具调用会在冷恢复时按 interrupted 修复。优先前滚；必须回滚时，先优雅停止 v0.9.7，并把所有遗留 v0.9.4 卡片视为 stale。 |
