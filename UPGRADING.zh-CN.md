...
| `0.8.1`–`0.8.2` | 与 v0.8.0 相比没有插件自有持久化 schema 变化。 | 在精确 rc.6 版本组上可与 v0.8.0 共用 v2 状态，但仍应遵守冷备份规则。 |
...
| 从 v0.8.2 回滚到 | 状态处理方式 |
| --- | --- |
| v0.8.1 或 v0.8.0 | 使用相同 v2 binding schema。在精确 rc.6 版本组上，优雅停机并保留快照后可以原地回滚代码。 |
