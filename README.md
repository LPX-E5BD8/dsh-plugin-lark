...
Inbound images, outbound artifact sending, URLs, archives, generic binaries, audio, and video remain unsupported in v0.9.8 and are split into later reviewable Roadmap milestones. Image-aware routing applies to image blocks that already entered a shared Harness Session through another trusted surface; it does not download or persist image bytes itself. Group attachments also remain fail-closed: ordinary standalone file messages cannot carry the bot mention required by the channel, so unmentioned files stay silent and a synthetic/explicitly mentioned non-text event receives only the generic notice. An administration UI and generic card framework remain intentionally out of scope.
