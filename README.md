...
5. Grant `im:resource` when `inboundTextFiles` or `inboundImages` is enabled. Without either option, the scope remains optional and only enables the bundled animated loading indicator; the card otherwise uses a static icon.
...
Outbound artifact sending, URLs, archives, generic binaries, audio, video, GIF, WebP, animated/multi-image PNG/JPEG, and every group image remain unsupported in v0.9.9 and are split into later reviewable Roadmap milestones. Group attachments remain fail-closed: ordinary standalone media messages cannot carry the bot mention required by the channel, so unmentioned files stay silent and a synthetic/explicitly mentioned non-text event receives only the generic notice. An administration UI and generic card framework remain intentionally out of scope.
