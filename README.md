...
`documentHandoff` is off by default and needs its own Lark app scopes: `docx:document:readonly` to read and `docx:document:create` to publish. Enabling it registers two Agent-scoped tools and nothing else; the Docs, Calendar, Base, Sheets, Tasks, Wiki, and Drive surfaces stay outside this channel. The read tool accepts only an absolute `https` link to a `docx` or `wiki` path on one of the deployment's own Lark hosts, exactly as the user supplied it -- a bare token, a relative path, credentials in the authority, a lookalike host, and a `drive`/`base`/`sheets` link are all refused before any request is made. Content is bounded by `maxDocumentReadBytes` on a character boundary and returned with its title, link, a truncation notice when one applies, and an explicit statement that it is untrusted data rather than an instruction. The publish tool creates one document from Markdown and returns its link; the ordinary chat answer and its delivery receipt are unchanged, so publishing adds a document rather than replacing the reply.
