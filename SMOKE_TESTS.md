...
13. In a disposable fault-injection run, let a `/new`, `/project`, or `/model` mutation commit while suppressing its inbound receipt, restart, and redeliver the exact platform message. Verify the generation or model does not change and the command is acknowledged. Apply a later mutation, redeliver the earlier message again, and verify the latest project/model remains current.
