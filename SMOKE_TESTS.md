...
4. Grant the bot permission to receive and send messages. Grant `im:resource` when inbound text files or the animated loading image are being checked.
...
5. Every group attachment remains fail-closed. A normal unmentioned file stays silent and is never downloaded; if a controlled event fixture supplies an explicit mention on a non-text event, it receives only the generic notice and still performs no download. Credential-backed attachment success is claimed only for direct chats in v0.9.6.
...
- UTC timestamp;
- pass/fail for startup, direct chat, group chat, bounded text attachments, cards, structured input, commands, approvals, long reply, restart, project switching, session navigation, model switching, and bounded-cache cold resume;
